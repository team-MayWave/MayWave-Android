...
data class ChatGameUiState(
    ...
    fun isResultReadyFor(request: ChatGameRequest): Boolean {
        val requestKey = request.key

        return responseMessages[requestKey] != null || errorMessages[requestKey] != null
    }

    fun hasErrorFor(request: ChatGameRequest): Boolean {
        return errorMessages[request.key] != null
    }
